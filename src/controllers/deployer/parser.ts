...
export function mergeEnvironments(
  defaultEnv: IDeployer,
  selectedEnv: IDeployer
): IDeployer {
  ...
  mergedEnvData.ConfigMap.name =
    selectedEnv.ConfigMap?.name ?? defaultEnv.ConfigMap?.name;
...
export function getMergedEnvironment(envName: string): IDeployer {
  const selectedEnv = system.openJsonFile(
    `./assets/environment/${envName}.json`
  ) as IDeployer;
...
export async function parseEnvironmentDataOptions(
  ...
  for (const obj of data) {
    ...
    if (
      obj.type === DataTypeEnum.Random ||
      (localOnly && obj.type === DataTypeEnum.Prompt)
    ) {
...
export async function getLocalConfigMapDict(
  ...
  if (options?.filterByKey?.length ?? 0 > 0) {
    envDataCopy.ConfigMap.data = envDataCopy.ConfigMap.data.filter(
      (obj: IData) => (options?.filterByKey as string[]).includes(obj.key)
    );
  }
  if (options?.filterByType?.length ?? 0 > 0) {
    envDataCopy.ConfigMap.data = envDataCopy.ConfigMap.data.filter(
      (obj: IData) => options?.filterByType?.some((term) => obj.type === term)
    );
  }
  if (options?.filterByProperty?.length ?? 0 > 0) {
    envDataCopy.ConfigMap.data = envDataCopy.ConfigMap.data.filter(
      (obj: IData) =>
        options?.filterByProperty?.some((term) =>
          Object.prototype.hasOwnProperty.call(obj, term)
        )
    );
  }
  if (options?.exclude?.length ?? 0 > 0) {
    envDataCopy.ConfigMap.data = envDataCopy.ConfigMap.data.filter(
      (obj: IData) => options?.exclude?.every((term) => obj.key !== term)
    );
  }
  // Parse the ConfigMap data options
  return await parseEnvironmentDataOptions(
    envDataCopy.ConfigMap.data,
...
export async function getLocalChartsValues(
  ...
  charts =
    options?.find?.length ?? 0 > 0
      ? charts.filter((el) =>
          options?.find?.some((term) => el.name.includes(term))
        )
      : charts;
...
export function getEnvironments(type = ".json"): string[] {
  ...
  let files = system
    .getFilesInDir("./assets/environment")
    .filter((el) => el.includes(type));
...
export function getKeysListByProp(
  envData: IDeployer,
  property: "display" | "lock"
): string[] {
  const settingsValues = envData.Settings.filter((x) => x[property]).map(
    (x) => x.key
  );
  const configMapValues = envData.ConfigMap.data
    .filter((x) => x[property])
    .map((x) => x.key);
  const secretValues = envData.Secret.data
    .filter((x) => x[property])
    .map((x) => x.key);
